...
// File: contracts/interfaces/IChiliSwapFactory.sol
...
// File: contracts/interfaces/IChiliSwapOracle.sol



pragma solidity 0.6.12;

interface IChiliSwapOracle {
    function update(address tokenA, address tokenB) external;

    function consult(address tokenIn, uint amountIn, address tokenOut) external view returns (uint amountOut);
}

// File: contracts/interfaces/IChiliSwapRouter.sol



pragma solidity 0.6.12;

interface IChiliSwapRouter {
    function factory() external pure returns (address);
}

// File: contracts/interfaces/IChiliSwapToken.sol
...
interface IChiliSwapToken is IERC20 {
    /**
...
contract Schedule is Ownable {
    ...
    uint256 public epochStartBlock = 0;          //the block number when chiliswap starts
    uint256 public chiliswapPerBlock = 0;           //chiliswap tokens created per block
    ...
    event ChiliSwapPerBlockTransferred(uint256 indexed previousChiliSwapPerBlock, uint256 indexed newChiliSwapPerBlock);

    constructor(uint256 _chiliswapPerBlock) public {
        emit ChiliSwapPerBlockTransferred(chiliswapPerBlock, _chiliswapPerBlock);
        chiliswapPerBlock = _chiliswapPerBlock;
    }
    ...
    function setChiliSwapPerBlock(uint256 _chiliswapPerBlock) public virtual onlyOwner {
        emit ChiliSwapPerBlockTransferred(chiliswapPerBlock, _chiliswapPerBlock);
        chiliswapPerBlock = _chiliswapPerBlock;
    }
    ...
    function reward(uint256 blockNumber) public view returns (uint256) {
        ...
        return chiliswapPerBlock.mul(numerator).div(denominator);
    }
...
// File: contracts/ChiliSwapTrading.sol
...
contract ChiliSwapTrading is Schedule, AccessControl {
    ...
    struct PoolInfo {
        ...
        uint256 allocChiliSwapAmount;  // How many CHILIs
    }

    // The chiliswap token
    IChiliSwapToken public chiliswap;
    // The swap router
    IChiliSwapRouter public router;
    // The swap factory
    IChiliSwapFactory public factory;
    // The price oracle
    IChiliSwapOracle public oracle;
    // The base coin
    address public usdt;
    ...
    // The block number when chiliswap mining starts.
    ...
    constructor(
        IChiliSwapToken _chiliswap,
        IChiliSwapRouter _router,
        IChiliSwapOracle _oracle,
        address _usdt,
        uint256 _chiliswapPerBlock,
        uint256 _startBlock
    ) public Schedule(_chiliswapPerBlock) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());

        chiliswap = _chiliswap;
        router = _router;
        factory = IChiliSwapFactory(router.factory());
        oracle = _oracle;
        usdt = _usdt;
    ...
    function checkPoolDuplicate(address _lpToken) internal view {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].lpToken != _lpToken, "ChiliSwapTrading::add: existing pool");
        }
    }
    ...
    function add(uint256 _allocPoint, address _lpToken, bool _withUpdate) public onlyOwner {
        ...
        quantity : 0,
        totalQuantity : 0,
        allocChiliSwapAmount : 0
        }));

        pairOfPid[_lpToken] = poolLength() - 1;
    }

    // Update the given pool's ChiliSwap allocation point. Can only be called by the owner.
    ...
    function setChiliSwapPerBlock(uint256 _chiliswapPerBlock) public virtual override onlyOwner {
        massUpdatePools();
        super.setChiliSwapPerBlock(_chiliswapPerBlock);
    }

    function setRouter(IChiliSwapRouter _router) public onlyOwner {
        require(address(_router) != address(0), "ChiliSwapTrading::setRouter: _router is the zero address");
        router = _router;
        factory = IChiliSwapFactory(router.factory());
    }

    function setOracle(IChiliSwapOracle _oracle) public onlyOwner {
        require(address(_oracle) != address(0), "ChiliSwapTrading::setOracle: _oracle is the zero address");
        oracle = _oracle;
    }
    ...
    function pendingChiliSwap(uint256 _pid) external view returns (uint256) {
        require(_pid <= poolInfo.length - 1, "ChiliSwapTrading::pendingChiliSwap: not find this pool");

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 allocChiliSwapAmount = pool.allocChiliSwapAmount;
        if (user.quantity > 0) {
            uint256 blockReward = mintable(pool.lastRewardBlock);
            uint256 chiliswapReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
            allocChiliSwapAmount = allocChiliSwapAmount.add(chiliswapReward);
            return user.quantity.mul(allocChiliSwapAmount).div(pool.quantity);
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 chiliswapReward = blockReward.mul(pool.allocPoint).div(totalAllocPoint);
        chiliswap.mint(address(this), chiliswapReward);
        pool.allocChiliSwapAmount = pool.allocChiliSwapAmount.add(chiliswapReward);
        pool.lastRewardBlock = block.number;
    }

    function swap(address account, address input, address output, uint256 amount) public onlyRouter returns (bool) {
        require(account != address(0), "ChiliSwapTrading::swap: taker swap account is the zero address");
        require(input != address(0), "ChiliSwapTrading::swap: taker swap input is the zero address");
        require(output != address(0), "ChiliSwapTrading::swap: taker swap output is the zero address");
        ...
        address pair = IChiliSwapFactory(factory).getPair(input, output);
    ...
    function withdraw() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (user.quantity > 0) {
                ...
                uint256 userReward = pool.allocChiliSwapAmount.mul(user.quantity).div(pool.quantity);
                pool.quantity = pool.quantity.sub(user.quantity);
                pool.allocChiliSwapAmount = pool.allocChiliSwapAmount.sub(userReward);
                user.quantity = 0;
                user.blockNumber = block.number;
        ...
        safeChiliSwapTransfer(msg.sender, userSub);
    }
    ...
    // Safe chiliswap transfer function, just in case if rounding error causes pool to not have enough CHILIs.
    function safeChiliSwapTransfer(address _to, uint256 _amount) internal {
        uint256 chiliswapBal = chiliswap.balanceOf(address(this));
        if (_amount > chiliswapBal) {
            chiliswap.transfer(_to, chiliswapBal);
        } else {
            chiliswap.transfer(_to, _amount);
        }
    }

    modifier onlyRouter() {
        require(msg.sender == address(router), "ChiliSwapTrading::onlyRouter: caller is not the router");
        _;
    }
}
