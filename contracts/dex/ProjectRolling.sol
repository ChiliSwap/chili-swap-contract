...
// File: contracts/interfaces/IChiliSwapPair.sol
...
// File: contracts/interfaces/IChiliSwapRouter01.sol
...
// File: contracts/interfaces/IChiliSwapRouter02.sol
...
// File: contracts/interfaces/IChiliSwapShaker.sol



pragma solidity 0.6.12;

interface IChiliSwapShaker {
...
// File: contracts/ChiliSwapRolling.sol
...
contract ChiliSwapRolling is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IChiliSwapRouter02 public immutable ROUTER;
    IChiliSwapShaker public immutable SHAKER;
    ...
    constructor(address _ROUTER, address _SHAKER, address _WKCS) public {
        ROUTER = IChiliSwapRouter02(_ROUTER);
        SHAKER = IChiliSwapShaker(_SHAKER);
        WKCS = _WKCS;
    }
    ...
    function zapInToken(address _from, uint256 amount, address _to) external {
        ...
        if (isLP(_to)) {
            IChiliSwapPair pair = IChiliSwapPair(_to);
    ...
    function zapOut(address _from, uint256 amount) external {
        ...
        if (!isLP(_from)) {
            ...
            IChiliSwapPair pair = IChiliSwapPair(_from);
            address token0 = pair.token0();
            address token1 = pair.token1();
    ...
    function _swapKCSToLP(address lp, uint256 amount, address receiver) private {
        if (!isLP(lp)) {
            ...
            // lp
            IChiliSwapPair pair = IChiliSwapPair(lp);
            address token0 = pair.token0();
            address token1 = pair.token1();
            if (token0 == WKCS || token1 == WKCS) {
