...
// File: contracts/ChiliSwapProfile.sol
/** @title ChiliSwapProfile.
@dev It is a contract for users to bind their address
to a customizable profile by depositing a NFT.
*/
contract ChiliSwapProfile is AccessControl, ERC721Holder, ReentrancyGuard {
    ...
    uint256 public numberChiliSwapToReactivate;
    uint256 public numberChiliSwapToRegister;
    uint256 public numberChiliSwapToUpdate;
    ...
    modifier onlyOwner() {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ChiliSwapProfile::onlyOwner: Not the main admin");
        _;
    }

    // Modifier for point roles
    modifier onlyPoint() {
        require(hasRole(POINT_ROLE, _msgSender()), "ChiliSwapProfile::onlyPoint: Not a point admin");
        _;
    }

    // Modifier for special roles
    modifier onlySpecial() {
        require(hasRole(SPECIAL_ROLE, _msgSender()), "ChiliSwapProfile::onlySpecial: Not a special admin");
        _;
    }
    ...
    constructor(
        IKRC20 _chiliswapToken,
        uint256 _numberChiliSwapToReactivate,
        uint256 _numberChiliSwapToRegister,
        uint256 _numberChiliSwapToUpdate
    ) public {
        chiliswapToken = _chiliswapToken;
        numberChiliSwapToReactivate = _numberChiliSwapToReactivate;
        numberChiliSwapToRegister = _numberChiliSwapToRegister;
        numberChiliSwapToUpdate = _numberChiliSwapToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends CHILI to burn address. Requires 2 token approvals.
     */
    function createProfile(uint256 _teamId, address _nftAddress, uint256 _tokenId) external nonReentrant {
        require(!hasRegistered[_msgSender()], "ChiliSwapProfile::createProfile: Already registered");
        require((_teamId <= numberTeams) && (_teamId > 0), "ChiliSwapProfile::createProfile: Invalid teamId");
        require(teams[_teamId].isJoinable, "ChiliSwapProfile::createProfile: Team not joinable");
        require(hasRole(NFT_ROLE, _nftAddress), "ChiliSwapProfile::createProfile: NFT address invalid");

        // Loads the interface to deposit the NFT contract
        IERC721 nftToken = IERC721(_nftAddress);

        require(_msgSender() == nftToken.ownerOf(_tokenId), "ChiliSwapProfile::createProfile: Only NFT owner can register");

        // Transfer NFT to this contract
        nftToken.safeTransferFrom(_msgSender(), address(this), _tokenId);

        // Transfer CHILI tokens to this contract
        chiliswapToken.safeTransferFrom(_msgSender(), address(this), numberChiliSwapToRegister);
    ...
    function pauseProfile() external nonReentrant {
        require(hasRegistered[_msgSender()], "ChiliSwapProfile::pauseProfile: Has not registered");

        // Checks whether user has already paused
        require(users[_msgSender()].isActive, "ChiliSwapProfile::pauseProfile: User not active");
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external nonReentrant {
        require(hasRegistered[_msgSender()], "ChiliSwapProfile::updateProfile: Has not registered");
        require(hasRole(NFT_ROLE, _nftAddress), "ChiliSwapProfile::updateProfile: NFT address invalid");
        require(users[_msgSender()].isActive, "ChiliSwapProfile::updateProfile: User not active");
        ...
        require(_msgSender() == nftNewToken.ownerOf(_tokenId), "ChiliSwapProfile::updateProfile: Only NFT owner can update");

        // Transfer token to new address
        nftNewToken.safeTransferFrom(_msgSender(), address(this), _tokenId);

        // Transfer CHILI token to this address
        chiliswapToken.safeTransferFrom(_msgSender(), address(this), numberChiliSwapToUpdate);
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external nonReentrant {
        require(hasRegistered[_msgSender()], "ChiliSwapProfile::reactivateProfile: Has not registered");
        require(hasRole(NFT_ROLE, _nftAddress), "ChiliSwapProfile::reactivateProfile: NFT address invalid");
        require(!users[_msgSender()].isActive, "ChiliSwapProfile::reactivateProfile: User is active");

        // Interface to deposit the NFT contract
        IERC721 nftToken = IERC721(_nftAddress);
        require(_msgSender() == nftToken.ownerOf(_tokenId), "ChiliSwapProfile::reactivateProfile: Only NFT owner can update");

        // Transfer NFT to contract
        nftToken.safeTransferFrom(_msgSender(), address(this), _tokenId);

        // Transfer to this address
        chiliswapToken.safeTransferFrom(_msgSender(), address(this), numberChiliSwapToReactivate);
    ...
    function increaseUserPointsMultiple(address[] calldata _userAddresses, uint256 _numberPoints, uint256 _campaignId) external onlyPoint {
        require(_userAddresses.length < 1001, "ChiliSwapProfile::increaseUserPointsMultiple: Length must be < 1001");
    ...
    function removeUserPointsMultiple(address[] calldata _userAddresses, uint256 _numberPoints) external onlyPoint {
        require(_userAddresses.length < 1001, "ChiliSwapProfile::removeUserPointsMultiple: Length must be < 1001");
        for (uint256 i = 0; i < _userAddresses.length; i++) {
    ...
    function addNftAddress(address _nftAddress) external onlyOwner {
        require(IERC721(_nftAddress).supportsInterface(0x80ac58cd), "ChiliSwapProfile::addNftAddress: Not ERC721");
    ...
    function addTeam(string calldata _teamName, string calldata _teamDescription) external onlyOwner {
        ...
        require(strBytes.length < 20, "ChiliSwapProfile::addTeam: Must be < 20");
        require(strBytes.length > 3, "ChiliSwapProfile::addTeam: Must be > 3");
    ...
    function changeTeam(address _userAddress, uint256 _newTeamId) external onlySpecial {
        require(hasRegistered[_userAddress], "ChiliSwapProfile::changeTeam: Has not registered");
        require((_newTeamId <= numberTeams) && (_newTeamId > 0), "ChiliSwapProfile::changeTeam: Invalid teamId");
        require(teams[_newTeamId].isJoinable, "ChiliSwapProfile::changeTeam: Team not joinable");
        require(users[_userAddress].teamId != _newTeamId, "ChiliSwapProfile::changeTeam: Already in the team");
    ...
    function makeTeamJoinable(uint256 _teamId) external onlyOwner {
        require((_teamId <= numberTeams) && (_teamId > 0), "ChiliSwapProfile::makeTeamJoinable: Invalid teamId");
    ...
    function makeTeamNotJoinable(uint256 _teamId) external onlyOwner {
        require((_teamId <= numberTeams) && (_teamId > 0), "ChiliSwapProfile::makeTeamNotJoinable: Invalid teamId");
    ...
    function renameTeam(uint256 _teamId, string calldata _teamName, string calldata _teamDescription) external onlyOwner {
        require((_teamId <= numberTeams) && (_teamId > 0), "ChiliSwapProfile::renameTeam: Invalid teamId");

        // Verify length is between 3 and 20
        bytes memory strBytes = bytes(_teamName);
        require(strBytes.length < 20, "ChiliSwapProfile::renameTeam: Must be < 20");
        require(strBytes.length > 3, "ChiliSwapProfile::renameTeam: Must be > 3");
    ...
    /**
     * @dev Update the number of CHILI to register
     * Callable only by owner admins.
     */
    function updateNumberChiliSwap(uint256 _newNumberChiliSwapToReactivate, uint256 _newNumberChiliSwapToRegister, uint256 _newNumberChiliSwapToUpdate) external onlyOwner {
        numberChiliSwapToReactivate = _newNumberChiliSwapToReactivate;
        numberChiliSwapToRegister = _newNumberChiliSwapToRegister;
        numberChiliSwapToUpdate = _newNumberChiliSwapToUpdate;
    }
    ...
    {
        require(hasRegistered[_userAddress], "ChiliSwapProfile::getUserProfile: Has not registered");
        return (
    ...
    {
        require((_teamId <= numberTeams) && (_teamId > 0), "ChiliSwapProfile::getTeamProfile: Invalid teamId");
        return (
...
contract IFOInitializable is IIFOV2, ReentrancyGuard, Ownable {
    ...
    // ChiliSwapProfile
    ChiliSwapProfile public chiliswapProfile;
     ...
     * @param _chiliswapProfileAddress: the address of the ChiliSwapProfile
    ...
    function initialize(
        ...
        address _chiliswapProfileAddress,
        uint256 _startBlock,
        uint256 _endBlock,
        ...
        chiliswapProfile = ChiliSwapProfile(_chiliswapProfileAddress);
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        require(chiliswapProfile.getUserStatus(msg.sender), "IFOInitializable::depositPool: Must have an active profile");
    ...
    function _claimPoints(address _user) internal {
        if (!_hasClaimedPoints[_user]) {
            ...
            if (sumPools > thresholdPoints) {
                ...
                chiliswapProfile.increaseUserPoints(msg.sender, numberPoints, campaignId);
            }
        }
    }
...
// File: contracts/ChiliSwapIFODeployer.sol
/**
 * @title ChiliSwapIFODeployer
 */
contract ChiliSwapIFODeployer is Ownable {
    using SafeERC20 for IKRC20;

    uint256 public constant MAX_BUFFER_BLOCKS = 5184000;

    address public immutable chiliswapProfile;
    ...
    /**
     * @notice Constructor
     * @param _chiliswapProfile: the address of the ChiliSwapProfile
     */
    constructor(address _chiliswapProfile) public {
        chiliswapProfile = _chiliswapProfile;
    }
    ...
    function createIFO(
        ...
        require(_lpToken != _offeringToken, "ChiliSwapIFODeployer::createIFO: Tokens must be different");
        require(_endBlock < (block.number + MAX_BUFFER_BLOCKS), "ChiliSwapIFODeployer::createIFO: EndBlock too far");
        require(_startBlock < _endBlock, "ChiliSwapIFODeployer::createIFO: StartBlock must be inferior to endBlock");
        require(_startBlock > block.number, "ChiliSwapIFODeployer::createIFO: StartBlock must be greater than current block");
    ...
    function recoverWrongTokens(address _tokenAddress) external onlyOwner {
        ...
        require(balanceToRecover > 0, "ChiliSwapIFODeployer::recoverWrongTokens: Balance must be > 0");
