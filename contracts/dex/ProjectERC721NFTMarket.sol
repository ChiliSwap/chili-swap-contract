...
// File contracts/ERC721NFTMarket.sol
pragma abicoder v2;
/**
...
contract ERC721NFTMarket is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    modifier onlyAdmin() {
        require(msg.sender == adminAddress, "ERC721NFTMarket::onlyAdmin: Not admin");
        _;
    }
     ...
     * @param _WKCSAddress: WKCS address
     * @param _minimumAskPrice: minimum ask price
     * @param _maximumAskPrice: maximum ask price
     */
    constructor(address _adminAddress, address _treasuryAddress, address _WKCSAddress, uint256 _minimumAskPrice, uint256 _maximumAskPrice) {
        require(_adminAddress != address(0), "ERC721NFTMarket::constructor: Admin address cannot be zero");
        require(_treasuryAddress != address(0), "ERC721NFTMarket::constructor: Treasury address cannot be zero");
        require(_WKCSAddress != address(0), "ERC721NFTMarket::constructor: WKCS address cannot be zero");
        require(_minimumAskPrice > 0, "ERC721NFTMarket::constructor: _minimumAskPrice must be > 0");
        require(_minimumAskPrice < _maximumAskPrice, "ERC721NFTMarket::constructor: _minimumAskPrice < _maximumAskPrice");
    ...
    /**
     * @notice Buy token with KCS by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     */
    function buyTokenUsingKCS(address _collection, uint256 _tokenId) external payable nonReentrant {
        // Wrap KCS
    ...
    /**
     * @notice Buy token with WKCS by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     * @param _price: price (must be equal to the askPrice set by the seller)
     */
    function buyTokenUsingWKCS(address _collection, uint256 _tokenId, uint256 _price) external nonReentrant {
        IKRC20(WKCS).safeTransferFrom(address(msg.sender), address(this), _price);
   ...
   * @param _withKCS: whether the token is bought with KCS (true) or WKCS (false)
   */
    function _buyToken(address _collection, uint256 _tokenId, uint256 _price, bool _withKCS) internal {
        require(_collections[_collection].status == CollectionStatus.Open, "ERC721NFTMarket::_buyToken: Not for trading");
        require(_askTokenIds[_collection].contains(_tokenId), "ERC721NFTMarket::_buyToken: Not for sale");

        Ask memory askOrder = _askDetails[_collection][_tokenId];

        // Front-running protection
        require(_price == askOrder.price, "ERC721NFTMarket::_buyToken: Incorrect price");
        require(msg.sender != askOrder.seller, "ERC721NFTMarket::_buyToken: Buyer cannot be seller");

        // Calculate the net price (collected by seller), trading fee (collected by treasury), creator fee (collected by creator)
        (uint256 netPrice, uint256 tradingFee, uint256 creatorFee) = _calculatePriceAndFeesForCollection(_collection, _price);
        ...
        // Transfer WKCS
        IKRC20(WKCS).safeTransfer(askOrder.seller, netPrice);
        ...
        // Emit event
        emit Trade(_collection, _tokenId, askOrder.seller, msg.sender, _price, netPrice, _withKCS);
    }
    ...
    function claimPendingRevenue() external nonReentrant {
        ...
        require(revenueToClaim != 0, "ERC721NFTMarket::claimPendingRevenue: Nothing to claim");
        pendingRevenue[msg.sender] = 0;

        IKRC20(WKCS).safeTransfer(address(msg.sender), revenueToClaim);
    ...
    function createAskOrder(address _collection, uint256 _tokenId, uint256 _askPrice) external nonReentrant {
        ...
        require(_askPrice >= minimumAskPrice && _askPrice <= maximumAskPrice, "ERC721NFTMarket::createAskOrder: Price not within range");

        // Verify collection is accepted
        require(_collections[_collection].status == CollectionStatus.Open, "ERC721NFTMarket::createAskOrder: Not for listing");

        // Verify token has restriction
        require(_canTokenBeListed(_collection, _tokenId), "ERC721NFTMarket::createAskOrder: TokenId not eligible");
    ...
    function modifyAskOrder(address _collection, uint256 _tokenId, uint256 _newPrice) external nonReentrant {
        ...
        require(_newPrice >= minimumAskPrice && _newPrice <= maximumAskPrice, "ERC721NFTMarket::modifyAskOrder: Price not within range");

        // Verify collection is accepted
        require(_collections[_collection].status == CollectionStatus.Open, "ERC721NFTMarket::modifyAskOrder: Not for listing");

        // Verify the sender has listed it
        require(_tokenIdsOfSellerForCollection[msg.sender][_collection].contains(_tokenId), "ERC721NFTMarket::modifyAskOrder: Token not listed");
    ...
    */
    function cancelAskOrder(address _collection, uint256 _tokenId) external nonReentrant {
        // Verify the sender has listed it
        require(_tokenIdsOfSellerForCollection[msg.sender][_collection].contains(_tokenId), "ERC721NFTMarket::cancelAskOrder: Token not listed");
     ...
     */
    function addCollection(address _collection, address _creator, address _whitelistChecker, uint256 _tradingFee, uint256 _creatorFee) external onlyAdmin {
        require(!_collectionAddressSet.contains(_collection), "ERC721NFTMarket::addCollection: Collection already listed");
        require(IERC721(_collection).supportsInterface(0x80ac58cd), "ERC721NFTMarket::addCollection: Not ERC721");
        require((_creatorFee == 0 && _creator == address(0)) || (_creatorFee != 0 && _creator != address(0)), "ERC721NFTMarket::addCollection: Creator parameters incorrect");
        require(_tradingFee + _creatorFee <= TOTAL_MAX_FEE, "ERC721NFTMarket::addCollection: Sum of fee must inferior to TOTAL_MAX_FEE");
    ...
    function closeCollectionForTradingAndListing(address _collection) external onlyAdmin {
        require(_collectionAddressSet.contains(_collection), "ERC721NFTMarket::closeCollectionForTradingAndListing: Collection not listed");
     ...
     * @dev Callable by admin
     */
    function modifyCollection(address _collection, address _creator, address _whitelistChecker, uint256 _tradingFee, uint256 _creatorFee) external onlyAdmin {
        require(_collectionAddressSet.contains(_collection), "ERC721NFTMarket::modifyCollection: Collection not listed");
        require((_creatorFee == 0 && _creator == address(0)) || (_creatorFee != 0 && _creator != address(0)), "ERC721NFTMarket::modifyCollection: Creator parameters incorrect");
        require(_tradingFee + _creatorFee <= TOTAL_MAX_FEE, "ERC721NFTMarket::modifyCollection: Sum of fee must inferior to TOTAL_MAX_FEE");
    ...
    function updateMinimumAndMaximumPrices(uint256 _minimumAskPrice, uint256 _maximumAskPrice) external onlyAdmin {
        require(_minimumAskPrice < _maximumAskPrice, "ERC721NFTMarket::updateMinimumAndMaximumPrices: _minimumAskPrice < _maximumAskPrice");
    ...
    function recoverFungibleTokens(address _token) external onlyOwner {
        require(_token != WKCS, "ERC721NFTMarket::recoverFungibleTokens: Cannot recover WKCS");
        uint256 amountToRecover = IKRC20(_token).balanceOf(address(this));
        require(amountToRecover != 0, "ERC721NFTMarket::recoverFungibleTokens: No token to recover");

        IKRC20(_token).safeTransfer(address(msg.sender), amountToRecover);
    ...
    function recoverNonFungibleToken(address _token, uint256 _tokenId) external onlyOwner nonReentrant {
        require(!_askTokenIds[_token].contains(_tokenId), "ERC721NFTMarket::recoverNonFungibleToken: NFT not recoverable");
    ...
    function setAdminAndTreasuryAddresses(address _adminAddress, address _treasuryAddress) external onlyOwner {
        require(_adminAddress != address(0), "ERC721NFTMarket::setAdminAndTreasuryAddresses: Admin address cannot be zero");
        require(_treasuryAddress != address(0), "ERC721NFTMarket::setAdminAndTreasuryAddresses: Treasury address cannot be zero");
     ...
     */
    function viewAsksByCollectionAndTokenIds(address collection, uint256[] calldata tokenIds) external view returns (bool[] memory statuses, Ask[] memory askInfo) {
     ...
     * @param size: size of the response
     */
    function viewAsksByCollectionAndSeller(address collection, address seller, uint256 cursor, uint256 size) external view returns (uint256[] memory tokenIds, Ask[] memory askInfo, uint256) {
        uint256 length = size;
     ...
     */
    function calculatePriceAndFeesForCollection(address collection, uint256 price) external view returns (uint256 netPrice, uint256 tradingFee, uint256 creatorFee) {
     ...
     */
    function canTokensBeListed(address _collection, uint256[] calldata _tokenIds) external view returns (bool[] memory listingStatuses) {
     ...
     */
    function _calculatePriceAndFeesForCollection(address _collection, uint256 _askPrice) internal view returns (uint256 netPrice, uint256 tradingFee, uint256 creatorFee) {
